...
class ChatMessageList:
    ...
    def __init__(self, messages: Optional[List[Union[Dict[str, str], ChatMessage]]] = None):
        """初始化消息列表
...
class OpenAI:
    ...
    def __init__(self, config: Optional[ConfigObject] = None):
        ...
        self.api_base = config.api_base if config and hasattr(config, 'api_base') else "https://api.openai.com/v1"
        self.model = config.model if config and hasattr(config, 'model') else "gpt-3.5-turbo"
    ...
    async def _request(self, method: str, endpoint: str, api_key: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """发送API请求的内部方法
    ...
    async def chat_completion(self, 
                             messages: Union[List[Dict[str, str]], ChatMessageList],
        ...
        # 处理消息列表格式
        messages_to_use = messages.to_list() if isinstance(messages, ChatMessageList) else messages
