...
fn plugin_descriptor() -> Descriptor {
    Descriptor {
        ...
        commands: vec![CommandListener {
            ...
            description: "pixiv artwork".into(),
            pattern: r"(?i)(?:https?://)?(?:www\.)?pixiv\.net/\S+|pid[:：\s]*([0-9]+)".into(),
            match_raw: true,
            handler: "Handle".into(),
        }],
        ..Default::default()
...
impl Plugin for Plug {
    ...
    async fn handle(
        ...
        let page_url = plugin_common::join_url(&pages, &format!("/pixiv/{key}"));
