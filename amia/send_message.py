...
class SendMessage:
    def __init__(self, messages: List["SendBaseMessage"]|"SendBaseMessage", bot: Amia, *, user_id: str|None=None, group_id: str|None=None) -> None:
...
class SendFileMessage(SendBaseMessage):
    def __init__(self, file_path: str, file_name: str|None=None) -> None:
        ...
        if file_name:
            data["name"] = file_name
        elif urlparse(file_path).scheme not in ('http', 'https', 'file') and not file_path.startswith('data:'):
...
class ForwardMessageItem:
    def __init__(self, user_id: str, nickname: str, content: List[SendBaseMessage]) -> None:
        """合并转发消息中的单条消息项
...
class SendForwardMessage(SendBaseMessage):
    def __init__(self, messages: List[ForwardMessageItem]) -> None:
        """发送合并转发消息
        ...
        """
        # 确保所有消息项都转换为字典格式
        formatted_messages = [msg.toDict() for msg in messages]
        
        super().__init__("forward", data={"messages": formatted_messages})
...
class SendCustomMusicMessage(SendBaseMessage):
    def __init__(self, title: str, desc: str, url: str, audio: str, cover: str) -> None:
        ...
        Args:
            ...
            cover: 封面图片链接
        """
        super().__init__("custom_music", data={
            "title": title,
            "desc": desc,
            "url": url,
            "audio": audio,
