...
var profileRegex = regexp.MustCompile(`^(?:(?P<server>cn|jp|tw|en|kr))?(?:个人信息|profile)$`)

// PJSKProfile 插件主结构
type PJSKProfile struct {
	mu  sync.RWMutex
	cfg struct {
...
func (p *PJSKProfile) Descriptor(ctx context.Context) (papi.Descriptor, error) {
	_ = ctx
	...
	return papi.Descriptor{
		Name:         "Amiabot PJSK Profile",
		PluginID:     "external.amiabot-pjsk-profile",
		Version:      "0.1.0",
		Author:       "nyanyabot",
		Description:  "PJSK 个人信息查询插件，发送 profile 截图",
		...
		Config: &papi.ConfigSpec{
			Version:     "1",
			Description: "Amiabot PJSK Profile plugin config",
			Schema:      schema,
			Default:     def,
		},
...
func (p *PJSKProfile) Configure(ctx context.Context, config json.RawMessage) error {
	_ = ctx
	cfg := struct {
		AmiabotPages  string `json:"amiabot_pages"`
		DefaultServer string `json:"default_server"`
...
func (p *PJSKProfile) Invoke(ctx context.Context, method string, paramsJSON json.RawMessage, callerPluginID string) (json.RawMessage, error) {
	_ = ctx
	_ = method
	_ = paramsJSON
	_ = callerPluginID
...
func (p *PJSKProfile) Shutdown(ctx context.Context) error {
	_ = ctx
	return nil
}

func (p *PJSKProfile) Status(ctx context.Context) (string, error) {
	return "OK", nil
}

func (p *PJSKProfile) handleProfile(ctx context.Context, eventRaw ob11.Event, match *papi.CommandMatch) (papi.HandleResult, error) {
	log := hclog.L()

	var evt map[string]any
	if err := json.Unmarshal(eventRaw, &evt); err != nil {
		log.Error("[Profile] 解析事件失败", "error", err)
		return papi.HandleResult{}, nil
	}
	...
	host := transport.Host()
	if host == nil {
		log.Warn("[Profile] host 为 nil，终止")
		return papi.HandleResult{}, nil
	}
	...
	pageURL := util.BuildPagesURL(pagesHost, "/pjsk/profile", map[string]string{
		"server": server,
		"id":     targetGameID,
	})
	...
	blobID := fmt.Sprintf("pjsk-profile-%s-%s-%d", server, targetGameID, time.Now().Unix())
...
func main() {
	plugin.Serve(&plugin.ServeConfig{
		...
		Plugins: plugin.PluginSet{
			transport.PluginName: &transport.Map{PluginImpl: &PJSKProfile{}},
		},
	})
}
